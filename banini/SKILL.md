...
本 repo（`banini-tracker`）內附 [`telegram-auth/`](../telegram-auth/)，提供獨立排程器與 Telegram 發送工具：

```bash
# 於 telegram-auth/ 內執行
python weekday_scheduler.py
```

`telegram-auth/banini_report.py` 以相對路徑 `../banini` 直接呼叫本 skill 的 `scripts/scrape_threads.py`，再以啟發式規則組裝 Telegram 友善格式的反指標報告。環境變數請見 `telegram-auth/.env.example`。

若改將本 skill 安裝到 `~/.claude/skills/banini/` 作為 Claude Code skill 使用，可另外搭配外部 [`skill-cron`](https://github.com/KerberosClaw/kc_ai_skills/tree/main/skill-cron) 註冊：
...
frontmatter 的 `headless-prompt` 即為 skill-cron 觸發時的執行內容。
...
- 互動模式用 `/banini`；非互動排程使用 `telegram-auth/weekday_scheduler.py` 或外部 skill-cron（`claude -p` 不支援 `/skill` 語法）
- 僅供娛樂參考，不構成投資建議
